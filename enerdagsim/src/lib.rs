...
use mosaik_rust_api::{ApiHelpers, AttributeId, Eid, Model, MosaikApi};
...
impl HouseholdDescription {
    pub fn random() -> HouseholdDescription {
        HouseholdDescription {
            household_type: "rand".to_string(),
            initial_energy_balance: random(),
            battery_config: BatteryConfigToml::default(),
            battery_type: HouseholdBatteries::SimpleBattery,
...
impl Neighborhood {
    ...
    ***********************************************/
    ...
    pub fn step(&mut self, _time: usize, inputs: HashMap<Eid, Map<AttributeId, Value>>) {
    ...
    pub(crate) fn reset_prosumption(&mut self) {
        self.households.par_iter_mut().for_each(|(_, household)| {
    ...
    fn calculate_grid_power_load(&mut self) -> i64 {
        self.grid_power_load = self
            .households
            .par_iter()
            .map(|(_, x)| x.get_power_load(&self.time))
            .sum();
    ...
    fn calculate_total_disposable_energy(&mut self) -> i64 {
        let time = self.time;
        let total_disposable_energy = self
            .households
            .par_iter()
            .map(|(_, household)| household.get_disposable_energy(&time))
            .sum();
    ...
    fn collect_market_bids(
        ...
        let bids: Vec<([u8; 32], enerdag_marketplace::bid::Bid)> = self
            .households
            .par_iter_mut()
            .map(|(name, household)| {
    ...
    fn search_household(&mut self, address: &AddressBytes) -> Option<&String> {
        self.households
            .par_iter()
            .map(|(key, _)| key)
            .find_any(|key| key.hash().eq(address))
    }
}
...
#[inline]
pub fn mw_to_k_wh(power_m_w: MW, time_in_s: f64) -> kWh {
    // P_MW * 1000 -> P_KW
    // P_KW * (T_secs / 3600) -> E_KW/H
    // Short: power_m_w * (time_in_s / 3.6)
