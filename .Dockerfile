...
ARG GIT_NAME="Docker Image"
ARG GIT_MAIL="you@example.com"
...
RUN ["/bin/bash", "-c", "echo / >> venv/lib/python3.8/site-packages/household.pth"]

COPY ./additional_datasets /additional_datasets

RUN  mkdir /data && mkdir /data/htw_berlin
# Extract HTW Berlin Dataset
RUN mv /htw_minimal.tar.zip /data/htw_berlin/
WORKDIR /data/htw_berlin
RUN ["/bin/bash", "-c", "tar -xvaf htw_minimal.tar.zip "]

# Extract reference predictions
RUN mkdir /data/reference_predictions && mkdir /data/reference_predictions/htw_berlin
WORKDIR /
RUN ["/bin/bash", "-c", "tar -xvaf /htw_berlin_sarima_predictions.tar.zip "]
RUN mv htw_berlin_result /data/reference_predictions/htw_berlin

# Create SQL Table (Compatible with simulationdatatabseconnection) for htw berlin and then create reference predicitons
RUN ["/bin/bash", "-c", "/venv/bin/python /additional_datasets/htw_berlin/create_dataset_csv.py -w /data/htw_berlin" ]
RUN ["/bin/bash", "-c", "/venv/bin/python /additional_datasets/create_reference_predictions.py -i /data/htw_berlin/dataset.csv -o /data/reference_predictions/htw_berlin -s htw_berlin -p B24 -p perfect balance"  ]
# Move COSSMIC Dataset, preprocess and create SQL Table,  then create reference predictions
RUN mkdir /data/preprocessed_householdsim
RUN mv /household_data_1min_singleindex.csv /data/preprocessed_householdsim
RUN ["/bin/bash", "-c", "/venv/bin/python /additional_datasets/preprocessed_householdsim/ground_truth_creator.py -i /data/preprocessed_householdsim/household_data_1min_singleindex.csv -o /data/preprocessed_householdsim/" ]
RUN mkdir /data/reference_predictions/preprocessed_householdsim
RUN ["/bin/bash", "-c", "/venv/bin/python /additional_datasets/create_reference_predictions.py -i /data/preprocessed_householdsim/dataset.csv -o /data/reference_predictions/preprocessed_householdsim -s preprocessed_householdsim -p B24 -p perfect balance" ]

# Checkout mosaik-rust-api battery branch and compile the marketplace example
WORKDIR /mosaik-rust-api
RUN ["/bin/bash", "-c", "git config --global  user.email '$GIT_MAIL' && git config --global  user.name '$GIT_NAME'"]
...
RUN mv target/release/examples/enerdag_sim /enerdag_sim

# Change WORKDIR back to simulation  repo
WORKDIR /
# When starting the container, run simulation
CMD /venv/bin/python /simulation/cosimulation_city_energy/rust_integrated_battery.py -d /data/htw_berlin/htw_dataset.sql -c /data/reference_predictions/htw_berlin -s /data/reference_predictions/htw_berlin/htw_berlin_result -o /simulation/data/sim_results
